...
import { Bot, Briefcase, LogOut, Menu, X } from "lucide-react";
        ...
        className={`fixed inset-y-0 left-0 z-50 transition-transform duration-300 transform ${
          isMobile
            ? sidebarOpen
              ...
              : "-translate-x-full"
            : "translate-x-0"
        } ${sidebarCollapsed ? "w-16" : "w-64"} bg-[#100f0f] border-r border-gray-200`}
      >
        {/* Company Logo */}
        ...
        <div className="absolute bottom-4 left-4 right-4">
          <button
            className="w-full flex items-center px-3 py-2.5 text-sm font-medium rounded-lg text-gray-900 transition-all duration-200"
            onClick={logout}
          >
            <LogOut className="h-5 w-5 flex-shrink-0 text-gray-400" />
            {!sidebarCollapsed && <span className="ml-3 text-gray-400" >Logout</span>}
          </button>
        </div>
      </div>
